<?php
	...
	function instert_flight( $flight , $data ,$db){
		...
		//Calculate possible flight reputation change
		$sql="SELECT * FROM reputation_system";
			if (!$result = $db->query($sql)) {
					die('There was an error running the query [' . $db->error . ']');
			}
			while ($row = $result->fetch_assoc()) {
				$reputation_vs_0_150=$row['reputation_vs_0_150'];
				$reputation_vs_150_300=$row['reputation_vs_150_300'];
				$reputation_vs_300_500=$row['reputation_vs_300_500'];
				$reputation_vs_500=$row['reputation_vs_500'];
				$pre_reputation_df=$row['reputation_default_flight'];
			}
		$pre_reputation=$pre_reputation_df;
		if($beacon_off=='1') { $pre_reputation=$pre_reputation-1; }
		if($ias_below_10000_ft=='1') { $pre_reputation=$pre_reputation-2; }
		if($lights_below_10000_ft=='1') { $pre_reputation=$pre_reputation-2; }
		if($lights_above_10000_ft=='1') { $pre_reputation=$pre_reputation-2; }
		if($stall=='1') { $pre_reputation=$pre_reputation-3; }
		if($overspeed=='1') { $pre_reputation=$pre_reputation-3; }
		if($refuel=='1') { $pre_reputation=$pre_reputation-5; }
		if($slew=='1') { $pre_reputation=$pre_reputation-8; }
		if($taxi_no_lights=='1') { $pre_reputation=$pre_reputation-3; }
		if($takeoff_ldn_lights_off=='1') { $pre_reputation=$pre_reputation-2; }
		if($landing_ldn_lights_off=='1') { $pre_reputation=$pre_reputation-2; }
		if($landed_not_in_planned=='1') { $pre_reputation=$pre_reputation-5; }
		if($taxi_speed=='1') { $pre_reputation=$pre_reputation-1; }
		if($qnh_takeoff=='1') { $pre_reputation=$pre_reputation-2; }
		if($qnh_landing=='1') { $pre_reputation=$pre_reputation-2; }

		$landing_vs_rep=abs($landing_vs);
		if($landing_vs_rep>0&&$landing_vs_rep<=150) {
			$pre_reputation=$pre_reputation-$reputation_vs_0_150;
		}elseif($landing_vs_rep>150&&$landing_vs_rep<=300) {
			$pre_reputation=$pre_reputation-$reputation_vs_150_300;
		}elseif($landing_vs_rep>300&&$landing_vs_rep<=500) {
			$pre_reputation=$pre_reputation-$reputation_vs_300_500;
		}elseif($landing_vs_rep>500) {
			$pre_reputation=$pre_reputation-$reputation_vs_500;
		}
		if($crash==1) { $pre_reputation=-1*$pre_reputation_df; }
