<?php
	...
	function calculate_cost ($gvauser_id,$flight_id,$flight_duration,$pax,$fuel,$distance,$db,$reporttype,$route_id,$cargo) {
		...
		$sql_cost = 'select * from financial_parameters where parameter_active=1';
		...
		while ($row_cost = $result_cost->fetch_assoc()) {
				...
				if ($row_cost['linked_to_time']==1){			
					...
					if ($row_cost["is_cost"]=='1') {
					$amount = -1 * $row_cost['amount'] * $flight_duration;
					}
					if ($row_cost["is_profit"]=='1') {
              $amount = $row_cost['amount'] * $flight_duration;
				...
				// Cost by distance
				if ($row_cost['linked_to_distance']==1 && $row_cost['linked_to_pax']==0){
					$para_id = $row_cost['id'];
              ...
              $amount = $row_cost['amount'] * $distance;
				...
				// Cost by distance and pax
				if ($row_cost['linked_to_distance']==1 && $row_cost['linked_to_pax']==1){
					$para_id = $row_cost['id'];
					$para_desc = $row_cost['financial_parameter'].'(Distance: '.$distance.' Pax: '.$pax.')';
					if ($row_cost["is_cost"]=='1') {
							$amount = -1 * $row_cost['amount'] * $distance * $pax;
					}
					if ($row_cost["is_profit"]=='1') {
              $amount = $row_cost['amount'] * $distance * $pax;
          }					if ($amount!=0)
					{
						$sql = "insert into va_finances (parameter_id,finance_date,amount,gvauser_id, description, report_type,report_id) values ($para_id,now(),$amount,$gvauser_id,'$para_desc','$reporttype','$flight_id')";
						if (!$result = $db->query($sql)) {
				...
				// Cost by pax
				if ($row_cost['linked_to_pax']==1 && $row_cost['linked_to_distance']==0){
					$para_id = $row_cost['id'];
              ...
              $amount = $row_cost['amount'] * $pax;
				...
				if ($row_cost['linked_to_fuel']==1){
					$para_id = $row_cost['id'];
					$para_desc = $row_cost['financial_parameter'].'('.$row_cost['amount'].'). Fuel used: '.$fuel.' kg.';
					if ($row_cost["is_cost"]=='1') {
		...
		//DESACTIVADO por la parte de arriba!!
		$sql_cost = 'select * from financial_parameters where parameter_active=1 and is_profit=2';
	...
	$sql = "select * from pirepfsfk where IDPIREP='" . $flight . "'";
	if (!$result = $db->query($sql)) {
		die('There was an error running the query [' . $db->error . ']');
	}
	while ($row = $result->fetch_assoc()) {
