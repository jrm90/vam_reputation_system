				...
				<div class="panel-heading"><IMG src="images/icons/ic_chat_white_18dp_1x.png">&nbsp;<?php echo VAMACARS_VALIDATION_COMMENTS; ?></div>
					<div class="container">
				      <form id="validate_comment_form" name="validate_comment_form" method="post">
						<div class="control-group">
							...
							<div class="controls">
								...
								<span class="add-on"><i class="icon-pencil"></i></span>
									<textarea id="comment" name="comment" class="span4" rows="4" cols="80" placeholder="Comment (Max 500 characters)"></textarea>
						...
						<div class="control-group">
						  <div class="controls">
							<button type="button" id="addcommentbtn" name ="addcommentbtn" class="btn btn-primary"><?php echo VAMACARS_VALIDATION_ADD_COMMENT; ?></button>
							<button type="button" id="acceptbtn" name ="acceptbtn" class="btn btn-success"><?php echo VAMACARS_VALIDATION_ACCEPT; ?></button>
							<button type="button" id="rejectbtn" name ="rejectbtn" class="btn btn-warning"><?php echo VAMACARS_VALIDATION_REJECT; ?></button>
							<button type="button" id="deletebtn" name ="deletebtn" class="btn btn-danger"><?php echo VAMACARS_VALIDATION_DELETE; ?></button>
					      </div>
						</div>
						<div class="control-group">
							<div id="result"></div>
	...
	<div class="col-md-12">
			<div class="panel panel-default">
				<div class="panel-heading"><span class="glyphicon glyphicon-thumbs-up" aria-hidden="true"></span>&nbsp;<?php echo PILOT_REP; ?></div>
					<div class="container">
					<?php
						$sql1 = "select * from vampireps vp ,gvausers u where u.gvauser_id = vp.gvauser_id and vp.flightid='" . $vamflightid . "'";
							if (!$result1 = $db->query($sql1)) {
								die('There was an error running the query  [' . $db->error . ']');
							}
						while ($row1 = $result1->fetch_assoc()) {
							$userid=$row1["gvauser_id"];
							$reputation=$row1["reputation"];
							$alter_reputation=$row1["alter_reputation"];
						}
					...
					?>
					<form class="form-inline" id="alter_reputation" action="./index_vam_op.php?page=alter_reputation_validator" role="form" method="post">
						<div class="form-group">
							<label for="alter_reputation" class="col-4 col-form-label">Alterar Rating del piloto: </label>
							<div class="col-8">
								<input class="form-control" type="number" name="alter_reputation" value="<?php echo $alter_reputation; ?>" min="-15" max="15" id="alter_reputation">
							</div>
						</div>
						<div class="form-group">
						...
						<input class="hidden" id="url" name="url" value="<?php echo $_SERVER["REQUEST_URL"]; ?>" hidden>
